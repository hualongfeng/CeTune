...
class Deploy_RGW(Deploy) :
    ...
    def deploy(self, ceph_disk=False):
        self.map_diff = self.cal_cephmap_diff()
        rgw_nodes = self.map_diff["radosgw"]
        #self.rgw_dependency_install()
        #self.rgw_install()
    ...
    def restart_rgw(self):
        ...
        for rgw_node in self.cluster['rgw']:
            ...
            stdout, stderr = common.pdsh(self.cluster['user'],[rgw_node],'host_name=`hostname -s`;for inst in {%s..%s}; do radosgw -n client.radosgw.${host_name}-$inst 2>/dev/null; done;' % (self.cluster['rgw_index'][index],self.cluster['rgw_index'][index_end-1]), option="console|check_return")
            index = index_end
    ...
    def check_rgw_runing(self):
        return True
#        common.printout('LOG',"http_proxy=%s curl -D - -H 'X-Auth-User: %s' -H 'X-Auth-Key: %s' %s" % (self.cluster["proxy"], self.cluster["auth_username"], self.cluster["auth_password"], self.cluster["auth_url"]))
...
#        return True
    ...
    def create_pools(self):
        ...
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool create .rgw.root 8 8 replicated', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool set .rgw.root size 2', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'while [ $(ceph -s | grep creating -c) -gt 0 ]; do echo -n .;sleep 1; done', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool application enable .rgw.root rgw', 'check_return')

        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool create default.rgw.data.root 8 8 replicated', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool create default.rgw.gc 8 8 replicated', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool create default.rgw.users.uid 8 8 replicated', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool create default.rgw.users.keys 8 8 replicated', 'check_return')

        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool create default.rgw.control 8 8 replicated', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool set default.rgw.control size 2', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'while [ $(ceph -s | grep creating -c) -gt 0 ]; do echo -n .;sleep 1; done', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool application enable default.rgw.control rgw', 'check_return')

        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool create default.rgw.log 8 8 replicated', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool set default.rgw.log size 2', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'while [ $(ceph -s | grep creating -c) -gt 0 ]; do echo -n .;sleep 1; done', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool application enable default.rgw.log rgw', 'check_return')

        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool create default.rgw.meta 512 512 replicated', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool set default.rgw.meta size 2', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'while [ $(ceph -s | grep creating -c) -gt 0 ]; do echo -n .;sleep 1; done', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool application enable default.rgw.meta rgw', 'check_return')

        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool create default.rgw.buckets.index 512 512 replicated', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool set default.rgw.buckets.index size 2', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'while [ $(ceph -s | grep creating -c) -gt 0 ]; do echo -n .;sleep 1; done', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool application enable default.rgw.buckets.index rgw', 'check_return')

        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool create default.rgw.buckets.non-ec 8 8 replicated', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool set default.rgw.buckets.non-ec size 2', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'while [ $(ceph -s | grep creating -c) -gt 0 ]; do echo -n .;sleep 1; done', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool application enable default.rgw.buckets.non-ec rgw', 'check_return')

        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool create default.rgw.buckets.data 512 512 replicated', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool set default.rgw.buckets.data size 2', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'while [ $(ceph -s | grep creating -c) -gt 0 ]; do echo -n .;sleep 1; done', 'check_return')
        common.pdsh(self.cluster['user'],self.cluster['rgw'],'ceph osd pool application enable default.rgw.buckets.data rgw', 'check_return')
#        common.pdsh(self.cluster['user'],self.cluster['rgw'],"until [ $(ceph osd pool get default.rgw.buckets.data pg_num 2>/dev/null | awk '{print $2}') -eq 1024 ] && [ $(ceph osd pool get default.rgw.buckets.data pgp_num 2>/dev/null | awk '{print $2}') -eq 2048 ]; do echo -n =; sleep 1; done", 'check_return')
    ...
    def init_auth(self):
        ...
        common.pdsh(self.cluster['user'],rgw_node,'radosgw-admin user create --uid="cosbench" --display-name="cosbench" --secret-key=intel2012 --access-key=intel2012', 'check_return')
        common.pdsh(self.cluster['user'],rgw_node,'radosgw-admin user create --uid="cosbench2" --display-name="cosbench2" --secret-key=intel20122 --access-key=intel20122', 'check_return')
        common.pdsh(self.cluster['user'],rgw_node,'radosgw-admin user create --uid="cosbench3" --display-name="cosbench3" --secret-key=intel20123 --access-key=intel20123', 'check_return')
        common.pdsh(self.cluster['user'],rgw_node,'radosgw-admin user create --uid="cosbench4" --display-name="cosbench4" --secret-key=intel20124 --access-key=intel20124', 'check_return')
        common.pdsh(self.cluster['user'],rgw_node,'radosgw-admin user create --uid="cosbench5" --display-name="cosbench5" --secret-key=intel20125 --access-key=intel20125', 'check_return')
        common.pdsh(self.cluster['user'],rgw_node,'radosgw-admin user create --uid="cosbench6" --display-name="cosbench6" --secret-key=intel20126 --access-key=intel20126', 'check_return')
    ...
    def gen_conf(self, rgw_nodes = None):
        ...
        while (total_rgw_ins - rgw_index + 1) > 0:
            ...
            conf.append("rgw frontends =beast port=%s\n" %(str(civetweb_port)))
            #conf.append("rgw frontends =civetweb port=%s\n" %(str(civetweb_port)))
